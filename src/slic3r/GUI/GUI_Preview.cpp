...
void Preview::load_print_as_fff(bool keep_z_range, bool only_gcode)
{
    ...
    //BBS: always load shell at preview
    // Skip shell loading under memory pressure (user acknowledged the
    // memory warning dialog). Shells mirror model triangle data into
    // GPU buffers; on a full-plate 500 MB+ 3MF this can trigger page
    // faults and OOM. Skipping keeps the preview page alive.
