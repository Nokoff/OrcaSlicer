...
public:
    ...
    // Save and spatially restore painted facets when an operation replaces this volume's mesh.
...
public:
    ...
    bool printable;
    // Whether or not this instance is automatically dropped onto the print bed after being transformed.
    bool auto_drop;
...
private:
    ...
    explicit ModelInstance(ModelObject* object) : print_volume_state(ModelInstancePVS_Inside), printable(true), auto_drop(true), object(object), m_assemble_initialized(false) { assert(this->id().valid()); }
    // Constructor, which assigns a new unique ID.
